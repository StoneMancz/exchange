<template>
    <div id="transfer-legal" v-if="coins.length">
       ...
       <div class="balance" style="line-height:60px;margin:20px 0 0px">{{$t('asset.microUse')}}：{{micro_coins[coinIndex].micro_balance}}</div>
       <div class="balance" style="margin:5px 0 30px">{{$t('asset.changeUselg')}}：{{coins[coinIndex].change_balance}}</div>
       <button type='button' class="transfer pointer" @click="transfer">{{$t('asset.transfer')}}</button>
    </div>
</template>

<script>
import { version } from '../../../static/tradeview/charting_library/charting_library.min';
export default {
  data() {
    return {
      token: "",
      ...
      coins: [],
      coinIndex: 0,
      types:[this.$t('asset.micro_account'),this.$t('asset.currency_account')],
      number:''
    };
  },
  created() {
    ...
    getCoins() {
      ...
      this.$ajax({
        ...
        if (res.data.type == "ok") {
          ...
          var data=res.data.message.micro_wallet.balance;
          var micro_coins = data.filter(function(item,index) {
                // console.log(item)
                return item.is_micro==1;
            }); 
            let coinsData=[];
            let microCoinsData=[];
            ...
            this.micro_coins=microCoinsData;
            this.coins=coinsData;
        }
      });
    },
    all(){
        if(this.types[0] == this.$t('asset.micro_account')){
            this.number = this.micro_coins[this.coinIndex].micro_balance
        }else{
            this.number = this.coins[this.coinIndex].change_balance
        }
    },
    transfer(){
        if(this.number == ''){
            ...
            // data.type = this.types[0] == this.$t('asset.micro_account')?2:1;
            data.currency_id = this.coins[this.coinIndex].currency;
            data.from_field=this.types[0] == this.$t("asset.micro_account") ? "micro" : "change",
            data.to_field=this.types[1] == this.$t("asset.currency_account") ? "change" : "micro"
            this.$ajax({
...
// WEBPACK FOOTER //
// src/view/accounts/transferMicro.vue