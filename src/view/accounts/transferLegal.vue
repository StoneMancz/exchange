<template>
    <div id="transfer-legal" v-if="coins.length">
       ...
       <div class="balance" style="line-height:60px;margin:20px 0 0 0px">{{$t('asset.changeUselg')}}：{{change_coins[coinIndex].change_balance}}</div>
       <div class="balance" style="margin:5px 0 30px 0">{{$t('asset.legalUse')}}：{{coins[coinIndex].legal_balance}}</div>
...
export default {
  data() {
    return {
      ...
      coins: [],
      coinIndex: 0,
      types:[this.$t('asset.currency_account'),this.$t('asset.fince_account')],
      number:''
    };
  },
  created() {
  ...
  methods: {
    getCoins() {
      ...
      this.$ajax({
        ...
        if (res.data.type == "ok") {
          ...
          var micro_coins = res.data.message.legal_wallet.balance.filter(item=>item.is_legal==1);
          ...
          let microCoinsData=[];
            ...
            this.change_coins=coinsData;
            this.coins=microCoinsData;
        }
      });
    },
    all(){
        if(this.types[0] == this.$t('asset.currency_account')){
            this.number = this.change_coins[this.coinIndex].change_balance
        }else{
    ...
    transfer(){
        if(this.number == ''){
            ...
            data.from_field=this.types[0] == this.$t('asset.currency_account') ? "change" : "legal";
            data.to_field=this.types[0] == this.$t('asset.currency_account') ? "legal" : "change";
            // data.type = this.types[0] == this.$t('asset.change_account')?4:3;
            this.$ajax({
...
// WEBPACK FOOTER //
// src/view/accounts/transferLegal.vue