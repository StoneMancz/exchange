<template>
	<div id="legaltrade-box" class="whites">
		<div class="buy-sell flex">
			...
			<div class="record light_blue" @click="recordList()">{{$t('fat.orderLog')}}</div>
		</div>
		<div class="list-box">
			...
			<ul class="list">
				<li v-for="(item,index) in list" :key="index" class="bdb flex bod_bc ft14 tc" v-if="item.is_shelves==1">
					<div class="flex alcenter">
						<div class="head" :style="{background:item.bgcolor}">{{item.seller_name || '' | strFirst}}</div>
					...
					<div class="flex alcenter center">{{item.limitation.min || '0.000' | toFixeds}} {{item.coin_code}} - {{item.limitation.max || '0.000' | toFixeds}} {{item.coin_code}}</div>
					<div class="flex alcenter price ft16 bold center">{{item.price || '0.000' | toFixeds}} {{item.coin_code}}</div>
					...
					<div class="flex alcenter end"  @click="buySell(item.price,item.limitation.min,item.limitation.max,item.id,item.type,item.surplus_number,item.coin_code)">
						<button class="curPer">{{classify}}{{name}}</button>
					</div>
				</li>
			</ul>
			<div class="more tc mt20 curPer" @click="getMore()" v-if="list.length&&more">{{$t('td.more')}}</div>
		...
		<div class="modal white" v-show="shows">
			...
			<div class="content">
				<div class="content-list">
					...
					<p class="price">{{$t('td.price')}}{{prices || '0.000' | toFixeds}} </p>
					<div class="trade">
						<p :class="['trade-name',{'active':types == 'trade'}]" @click="tabClassify(1)">{{coin}}{{$t('td.trade')}}</p>
						<p :class="['trade-num',{'active':types == 'num'}]" @click="tabClassify(2)">{{classify}}{{$t('td.num')}}</p>
					</div>
					<div class="totals-num">
						<input v-if=" types == 'trade' " class="number" type="number" :placeholder='$t("td.inwant")+money_type+$t("td.total")' v-model="nums">
						<input v-else class="number" type="number" :placeholder='$t("td.inwant")+money_type+$t("td.num")' v-model="nums">
						<button class="all curPer" type="button" v-if=" type== 'sell' " @click="allMoney();">{{$t('td.allbuy')}}</button>
						<button class="all curPer" type="button" v-else @click="allMoney();">{{$t('td.allsell')}}</button>
						<span class="name">{{name01}}</span>
					</div>
					<div class="maxnum">{{$t('td.limit')}}{{minNum || '0.000' | toFixeds}}-{{maxNum || '0.000' | toFixeds}}</div>
					<div class="trade-totals">
						<p class="total-price">{{$t('td.tradeTotal')}}</p>
						<p class="prices" v-if=" types == 'trade' ">{{nums || '0.000' | toFixeds}} {{coin}}</p>
						<p class="prices" v-else>{{nums * prices || '0.000' | toFixeds}} {{coin}}</p>
					</div>
					<!-- <p class="tip">请在24小时内联系商家付款，超出24小时将自动取消</p> -->
					<div class="btns">
						<p class="cannel">{{time}}{{$t('td.doceil')}}</p>
						<button class="comfirm curPer" type="button" @click="buyOrder()">{{$t('td.place')}}</button>
					</div>
				</div>
			</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			buyOrder() {
				...
				let token = window.localStorage.getItem("token") || "";
				layer.prompt({title: _this.$t('td.pwd'), formType: 1,btn:[_this.$t('td.confirm'), _this.$t('td.canceil')]}, function(pass, index){
					console.log(pass)
					_this.password=pass;
				if (_this.nums) {
